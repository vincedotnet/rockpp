...
class ControllerExtensionModuleNewslettersubscribe extends Controller {
	...
	public function index() {   
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/newslettersubscribe', 'token=' . $this->session->data['token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('extension/module/newslettersubscribe/unsubscribe', 'token=' . $this->session->data['token'] . '&user_id=' . $result['id'] . $url, true)
			);
		...
		$pagination->url = $this->url->link('extension/module/newslettersubscribe', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	public function unsubscribe() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			$this->redirect($this->url->link('extension/module/newslettersubscribe', 'token=' . $this->session->data['token'] . $url, true));
			...
			$this->response->redirect($this->url->link('extension/module/newslettersubscribe', 'token=' . $this->session->data['token'] . $url, true));
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!$json) {
				...
				if ($emails) {
					...
					if ($end < $email_total) {
						$json['next'] = str_replace('&amp;', '&', $this->url->link('extension/module/newslettersubscribe/send', 'token=' . $this->session->data['token'] . '&page=' . ($page + 1), true));
					} else {
						$json['next'] = '';
					}
